...
// TEMP one-time setter: writes a page's `fullHtml` (a pasted full-page design) into
// the site content via the service role. Token-gated. Non-destructive — only adds/
// clears fullHtml on the target page; all other content is preserved. REMOVE after use.
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ ok: true, siteSlug, pageSlug, htmlLen: html ? String(html).length : 0, totalPages: pages.length })
  } catch (e) {
