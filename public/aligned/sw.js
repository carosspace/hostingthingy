...
self.addEventListener('fetch', e => {
  ...
  if (url.origin !== self.location.origin || !url.pathname.startsWith('/aligned/')) return // /api + fonts → network
  const isDoc = req.mode === 'navigate' || req.destination === 'document' || url.pathname === '/aligned/' || url.pathname.endsWith('/index.html')
  if (isDoc) {
