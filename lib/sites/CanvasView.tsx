...
import { CANVAS_W, MOBILE_W, type PageCanvas, type CanvasElement } from './types'
...
export function CanvasView({ canvas, accent, siteSlug, contactEmail, safeHref, navPages }: ViewProps) {
  ...
  const inner = (el: CanvasElement, cqf: (px: number) => string) => {
    ...
    if (el.type === 'box')
      return <div style={{ width: '100%', height: '100%', background: el.fill || 'transparent', borderRadius: cqf(el.radius || 0), border: el.borderColor && el.borderWidth ? `${cqf(el.borderWidth)} solid ${el.borderColor}` : undefined }} />
    if (el.type === 'menu')
      return (
      ...
      <div className="md:hidden">
        {canvas.mobileCustom ? (
          <div style={{ ...bg, position: 'relative', width: '100%', aspectRatio: `${MOBILE_W} / ${Math.max(200, canvas.mobileH || canvas.h)}`, containerType: 'inline-size' } as CSSProperties}>
            {phoneEls.map(el => (
              <div key={el.id} style={{ position: 'absolute', left: cqm(el.mx ?? Math.round(el.x * MR)), top: cqm(el.my ?? Math.round(el.y * MR)), width: cqm(el.mw ?? Math.round(el.w * MR)), height: cqm(el.mh ?? Math.round(el.h * MR)), opacity: (el.opacity ?? 100) / 100, transform: el.rotate ? `rotate(${el.rotate}deg)` : undefined }}>
                {inner(el, cqm)}
              </div>
            ))}
          </div>
